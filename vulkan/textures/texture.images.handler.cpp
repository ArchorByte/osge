...
)
{
    ...
    for (const TextureImageInfo &info : texture_image_info)
    {
        ...
        // Create info for the texture image.
        VkImageCreateInfo image_info {};
        image_info.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
        image_info.imageType = VK_IMAGE_TYPE_2D;                                         // Type of the image (2D here).
        image_info.extent.width = static_cast<uint32_t>(image_width);                    // Pass the image width.
        image_info.extent.height = static_cast<uint32_t>(image_height);                  // Pass the image height.
        image_info.extent.depth = 1;                                                     // Select the image depth (1 as it's basic 2D).
        image_info.mipLevels = 1;                                                        // Amount of mitmaps.
        image_info.arrayLayers = 1;                                                      // Amount of array layers.
        image_info.format = VK_FORMAT_R8G8B8A8_SRGB;                                     // Format of the image (8 bits RGBA here).
        image_info.tiling = VK_IMAGE_TILING_OPTIMAL;                                     // Select the "optimal" tiling option.
        image_info.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;                            // Select the initial layout of the image.
        image_info.usage = VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT; // Pass the usage flags.
        image_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;                              // That texture image is not shared by queues.
        image_info.samples = VK_SAMPLE_COUNT_1_BIT;                                      // Disable multisampling.
        image_info.flags = 0;                                                            // Not using flags.

        VkImage texture_image = VK_NULL_HANDLE;
        VkResult image_creation = vkCreateImage(logical_device, &image_info, nullptr, &texture_image); // Try to create the texture image.
        ...
        if (!texture_image || texture_image == VK_NULL_HANDLE)
        {
        ...
        // Read the memory properties of the physical device.
        VkPhysicalDeviceMemoryProperties memory_properties;
        vkGetPhysicalDeviceMemoryProperties(physical_device, &memory_properties);

        // Read the memory requirements of the texture image.
        VkMemoryRequirements memory_requirements;
        vkGetImageMemoryRequirements(logical_device, texture_image, &memory_requirements);

        // Allocation info for the texture image.
        VkMemoryAllocateInfo allocation_info {};
        allocation_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        allocation_info.allocationSize = memory_requirements.size;
        allocation_info.memoryTypeIndex = find_memory_type(memory_requirements.memoryTypeBits, memory_properties, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

        VkDeviceMemory image_memory = VK_NULL_HANDLE;
        VkResult memory_allocation = vkAllocateMemory(logical_device, &allocation_info, nullptr, &image_memory); // Try to allocate the memory.
        ...
        if (!image_memory || image_memory == VK_NULL_HANDLE)
        {
        ...
        transition_image_layout(logical_device, command_pool, graphics_queue, texture_image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
        ...
        transition_image_layout(logical_device, command_pool, graphics_queue, texture_image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

        texture_images.emplace_back(image); // Register the texture image.
...
)
{
    ...
    // Destroy each texture image in the list.
    for (TextureImage &texture_image : texture_images)
    {
        i++;

        // Retrieve the texture image data.
        std::string texture_name = texture_image.name;
        VkImage image = texture_image.texture_image;
        VkDeviceMemory image_memory = texture_image.image_memory;

        if (!image || image == VK_NULL_HANDLE)
        {
            error_log("- Warning: The texture image #" + std::to_string(i) + "/" + std::to_string(texture_images.size()) + " (" + texture_name + ") destruction failed! The image provided (" + force_string(image) + ") is not valid!");
            failed++;
            continue;
        }
        ...
        if (!image_memory || image_memory == VK_NULL_HANDLE)
        {
            error_log("- Warning: The texture image #" + std::to_string(i) + "/" + std::to_string(texture_images.size()) + " (" + texture_name + ") destruction failed! The image memory provided (" + force_string(image_memory) + ") is not valid!");
            failed++;
            continue;
        }

        // Destroy the texture image and dispose of the address.
        vkDestroyImage(logical_device, image, nullptr);
        image = VK_NULL_HANDLE;

        // Free the texture image memory and dispose of the address.
    ...
    if (failed > 0) error_log("Warning: " + std::to_string(failed) + " texture images failed to destroy! This might lead to some memory leaks!");
