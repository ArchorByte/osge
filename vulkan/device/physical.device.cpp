...
)
{
    ...
    {
        fatal_error_log("Physical device research failed! The Vulkan instance provided (" + force_string(vulkan_instance) + ") is not valid!");
    }

    // Count the amount of physical device available on this device.
    ...
    {
        fatal_error_log("Physical device research failed! The physical devices query 1/2 returned error code " + std::to_string(first_devices_query) + ".");
    }

    if (devices_count == 0)
    {
        fatal_error_log("Physical device research failed! Failed to find any physical device with a driver supporting Vulkan!");
    }
    ...
    {
        fatal_error_log("Physical device research failed! Physical devices query 2/2 returned error code " + std::to_string(second_devices_query) + ".");
    }

    VkPhysicalDevice physical_device = VK_NULL_HANDLE;

    // Try to find a valid graphics card.
    ...
    if (!physical_device || physical_device == VK_NULL_HANDLE)
    {
        fatal_error_log("Physical device research failed! Failed to find any usable physical device!");
    }
